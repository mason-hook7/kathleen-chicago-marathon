...
# ─────────────────────────────────────────────────────────────
#  CROSS-DEVICE SYNC SETUP  (paste your two Supabase values here)
# ─────────────────────────────────────────────────────────────
# Leave as-is and the app saves progress on each device only.
# Paste your real values and progress + edited distances sync
# across every device. The anon key is a public browser-side key
# by design, so it's fine to keep here.
SUPABASE_URL      = "https://beafupyhaldiqzkuejvk.supabase.co"   # <-- your Project URL
SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJlYWZ1cHloYWxkaXF6a3VlanZrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA0NDI2MDMsImV4cCI6MjA5NjAxODYwM30.Q27zM0m-oX21oo5coG1t9JNY8I981Ok7bDArhVVLXlM"  # anon / public JWT
# ─────────────────────────────────────────────────────────────

# Interactive plan: Today banner, today-highlight, tap-to-complete,
# editable per-day mileage (pencil), live countdown, progress bar,
# and a cumulative miles-logged tracker. Persistence = browser cache
# + (if configured above) a shared Supabase record for all devices.
PLAN_HTML = r"""
<!DOCTYPE html>
<html lang="en">
<head>
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .count{display:flex;flex-direction:column;align-items:flex-start;border-right:1px solid var(--line);padding-right:22px;}
  .count .big{font-family:'Fraunces',serif;font-weight:900;font-size:clamp(28px,4.6vw,44px);line-height:.9;color:var(--race);}
  .count.miles .big{color:var(--done);}
  ...
  .prog-foot{display:flex;justify-content:space-between;align-items:center;gap:10px;}
  .sync{font-size:11px;color:var(--ink-soft);font-weight:600;}
  .reset{background:none;border:none;color:var(--ink-soft);font-family:'Hanken Grotesk',sans-serif;font-size:11px;font-weight:600;text-decoration:underline;cursor:pointer;padding:0;white-space:nowrap;}
  ...
  .edit-btn{position:absolute;top:9px;right:33px;font-size:12px;color:var(--ink-soft);cursor:pointer;opacity:.4;line-height:1;user-select:none;transition:opacity .12s;}
  .edit-btn:hover{opacity:1;color:var(--long);}
  .dist-input{width:52px;font-family:'Fraunces',serif;font-weight:900;font-size:17px;border:1.5px solid var(--long);border-radius:5px;padding:1px 4px;background:#fff;color:var(--ink);}
...
<div class="wrap">
  ...
  <div class="dash">
    ...
    <div class="count miles">
      <div class="big" id="miles-num">0</div>
      <div class="lab">miles logged</div>
      <div class="date" id="miles-sub">of 0 planned</div>
    </div>
    <div class="prog">
      <div class="prog-top">
      ...
      <div class="prog-foot">
        <span id="sync-status" class="sync"></span>
...
// ---------- persistence (local cache + optional cloud sync) ----------
const STORE_KEY = 'kc-marathon-progress-v1';

// Supabase config is injected by app.py. If left as the placeholders,
// the app silently runs in device-local mode (exactly like before).
const SB_URL = "__SUPABASE_URL__";
const SB_KEY = "__SUPABASE_KEY__";
const ROW_ID = "kathleen";
const sbConfigured = !!SB_URL && !!SB_KEY && SB_URL.indexOf('YOUR-PROJECT')===-1 && SB_KEY.indexOf('YOUR-ANON')===-1;

function normalize(raw){
  if(!raw || typeof raw!=='object') return { done:{}, miles:{} };
  if(raw.done || raw.miles) return { done: raw.done||{}, miles: raw.miles||{} };
  return { done: raw, miles: {} };   // back-compat: old flat {key:true} shape
}
function buildBlob(){ return { done: progress, miles: edits }; }
function cacheLocal(){ try { localStorage.setItem(STORE_KEY, JSON.stringify(buildBlob())); } catch(e){} }
function loadLocal(){ try { return normalize(JSON.parse(localStorage.getItem(STORE_KEY))); } catch(e){ return {done:{},miles:{}}; } }

function setSync(state){
  const el = document.getElementById('sync-status'); if(!el) return;
  const map = { ok:'✓ Synced across devices', syncing:'⟳ Syncing…', offline:'⚠ Offline — saved on this device', local:'Saved on this device only' };
...
async function remoteGet(){
  const r = await fetch(SB_URL+'/rest/v1/progress?id=eq.'+ROW_ID+'&select=data', { headers: sbHeaders() });
  if(!r.ok) throw new Error('GET '+r.status);
  const rows = await r.json();
  return (rows && rows[0] && rows[0].data) ? rows[0].data : {};
}
async function remotePut(blob){
  const r = await fetch(SB_URL+'/rest/v1/progress', {
    method:'POST',
    headers: sbHeaders({ 'Prefer':'resolution=merge-duplicates,return=minimal' }),
    body: JSON.stringify([{ id: ROW_ID, data: blob, updated_at: new Date().toISOString() }])
  });
  if(!r.ok) throw new Error('PUT '+r.status);
}
function persist(){
  cacheLocal();                    // instant local save (snappy + offline cache)
  if(sbConfigured){
    setSync('syncing');
    remotePut(buildBlob()).then(()=>setSync('ok')).catch(()=>setSync('offline'));
  } else {
    setSync('local');
  }
}
function getMiles(key, cell){
  if(edits[key]!==undefined) return edits[key];
  return Number(cell.dataset.orig);
}
function reapplyAll(){
  allCells.forEach(c=>{
    c.classList.toggle('done', progress[c.dataset.key]===true);
    if(c.classList.contains('editable') && !c.dataset.editing){
      c.querySelector('.d-dist').textContent = String(getMiles(c.dataset.key, c));
    }
  });
  updateTodayDist();
}
async function pullRemote(){
  if(!sbConfigured){ setSync('local'); return; }
  setSync('syncing');
  try {
    const remote = normalize(await remoteGet());   // cloud is the source of truth on load
    progress = remote.done; edits = remote.miles;
    cacheLocal();
    reapplyAll();
    refreshStats();
    syncBanner();
...
let progress = _local.done;
let edits = _local.miles;
...
weeks.forEach(w=>{
  ...
  const dayHTML = w.days.map((d,i)=>{
    ...
    const editable = checkable && /^[0-9]+(\.[0-9]+)?$/.test(String(dist));
    const key = 'W'+w.n+'-'+name;
    const dISO = isoLocal(dateForCell(w.n, i));
    DATEMAP[dISO] = {wn:w.n, name:name, type:type, dist:dist, label:label, distMod:distMod, checkable:checkable, editable:editable, key:key, dateObj:dateForCell(w.n,i)};
    const isDone = checkable && progress[key]===true;
    const showDist = (editable && edits[key]!==undefined) ? edits[key] : dist;
    return '<div class="day t-'+type+(checkable?' tappable':'')+(editable?' editable':'')+(isDone?' done':'')+'"'
      + ' data-date="'+dISO+'" data-week="'+w.n+'"'
      + (checkable ? ' data-key="'+key+'"' : '')
      + (editable ? ' data-orig="'+dist+'"' : '') + '>'
      + (checkable ? '<span class="check"></span>' : '')
      + (editable ? '<span class="edit-btn" title="Edit distance">&#9998;</span>' : '')
      + '<div class="d-name">'+name+'</div>'
      + '<div class="d-dist '+distMod+'">'+showDist+'</div>'
...
function toggleCell(cell){
  if(cell.dataset.editing) return;            // don't toggle while editing distance
...
// ----- edit distance (pencil) -----
function fmtNum(n){ const r = Math.round(n*10)/10; return (r%1===0) ? String(r) : r.toFixed(1); }
function setDistText(cell){
  cell.querySelector('.d-dist').textContent = fmtNum(getMiles(cell.dataset.key, cell));
  delete cell.dataset.editing;
}
function startEdit(cell){
  if(cell.dataset.editing) return;
  cell.dataset.editing = '1';
  const distEl = cell.querySelector('.d-dist');
  const cur = getMiles(cell.dataset.key, cell);
  distEl.innerHTML = '<input class="dist-input" type="number" step="0.1" min="0" value="'+cur+'">';
  const inp = distEl.querySelector('.dist-input');
  inp.focus(); inp.select();
  ['click','mousedown'].forEach(ev=> inp.addEventListener(ev, e=>e.stopPropagation()));
  inp.addEventListener('keydown', e=>{
    if(e.key==='Enter'){ e.preventDefault(); commitEdit(cell, inp.value); }
    else if(e.key==='Escape'){ setDistText(cell); }
  });
  inp.addEventListener('blur', ()=>commitEdit(cell, inp.value));
}
function commitEdit(cell, raw){
  if(!cell.dataset.editing) return;           // already committed (e.g. Enter then blur)
  const key = cell.dataset.key, orig = Number(cell.dataset.orig);
  let v = parseFloat(raw);
  if(isNaN(v) || v < 0){ setDistText(cell); return; }   // invalid → revert
  v = Math.round(v*10)/10;
  if(v === orig){ delete edits[key]; } else { edits[key] = v; }
  setDistText(cell);
  persist();
  refreshStats();
  if(todayInfo && todayInfo.key === key){ todayInfo.dist = String(v); const t=document.getElementById('tb-dist'); if(t) t.textContent = fmtNum(v)+' mi'; }
}
document.querySelectorAll('.edit-btn').forEach(btn=>{
  btn.addEventListener('click', e=>{ e.stopPropagation(); startEdit(btn.closest('.day')); });
});

function refreshStats(){
  let done = 0, milesDone = 0, milesPlan = 0;
  allCells.forEach(c=>{
    if(c.classList.contains('done')) done++;
    if(c.classList.contains('editable')){
      const m = getMiles(c.dataset.key, c);
      milesPlan += m;
      if(c.classList.contains('done')) milesDone += m;
    }
  });
  ...
  document.getElementById('miles-num').textContent = fmtNum(milesDone);
  document.getElementById('miles-sub').textContent = 'of ' + fmtNum(milesPlan) + ' planned';
...
document.getElementById('reset-btn').addEventListener('click', ()=>{
  if(confirm('Clear all checked workouts? (Edited distances are kept.)')){
    progress = {};
...
function initToday(){
  ...
  syncBanner();
  updateTodayDist();   // reflect any saved mileage edit for today
}

function updateTodayDist(){
  if(!todayInfo || !todayInfo.checkable) return;
  const c = document.querySelector('.day[data-key="'+todayInfo.key+'"]');
  if(c && c.classList.contains('editable')){
    const v = getMiles(todayInfo.key, c);
    todayInfo.dist = String(v);
    const t = document.getElementById('tb-dist'); if(t) t.textContent = fmtNum(v)+' mi';
  }
}
...
html = PLAN_HTML.replace("__SUPABASE_URL__", SUPABASE_URL).replace("__SUPABASE_KEY__", SUPABASE_ANON_KEY)

# height = how tall the scrollable plan frame is, in px (try 760–960).
